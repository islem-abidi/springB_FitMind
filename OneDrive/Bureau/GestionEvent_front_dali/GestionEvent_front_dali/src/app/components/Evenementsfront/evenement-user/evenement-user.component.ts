...
generateAndDownloadQRCode(eventId: number): void {
  ...
  this.eventService.generateQRCode(inscriptionId).subscribe({
    next: (response) => {
      ...
      // Mettre à jour le statut QR code généré
      this.loadEvents(); // Recharger les événements pour obtenir le statut à jour
